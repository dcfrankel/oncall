...
def filter_schedules(schedules: list[dict]) -> list[dict]:
    ...
    if JIRA_SERVICE_MANAGEMENT_FILTER_TEAM:
        filtered_schedules = []
        for s in schedules:
            if s["teamId"] == JIRA_SERVICE_MANAGEMENT_FILTER_TEAM:
            ...
            for rotation in schedule.get("rotations", []):
                for participant in rotation.get("participants", []):
                    if (
                        ...
                        and participant.get("id") in JIRA_SERVICE_MANAGEMENT_FILTER_USERS
                    ):
            ...
            if not include_schedule:
                for override in schedule.get("overrides", []):
                    if (
                        ...
                        and override.get("user", {}).get("id") in JIRA_SERVICE_MANAGEMENT_FILTER_USERS
                    ):
                        include_schedule = True
                        break
    ...
    if JIRA_SERVICE_MANAGEMENT_FILTER_SCHEDULE_REGEX:
        pattern = re.compile(JIRA_SERVICE_MANAGEMENT_FILTER_SCHEDULE_REGEX)
...
class Schedule:
    """
    Utility class for converting an Jira Service Management schedule to an OnCall schedule.
    An Jira Service Management schedule has multiple rotations, each with a set of participants.
    """
    ...
    def from_dict(cls, schedule: dict) -> "Schedule":
        """Create a Schedule object from an Jira Service Management API response for a schedule."""
        rotations = []
...
class Override:
    """
    Utility class for representing a schedule override in Jira Service Management.
    """
    ...
    def from_dict(cls, override: dict) -> "Override":
        """Create an Override object from an Jira Service Management API response for a schedule override."""
...
class Rotation:
    """
    Utility class for converting an Jira Service Management rotation to an OnCall shift.
    """
    ...
    def from_dict(cls, rotation: dict) -> "Rotation":
        """Create a Rotation object from an Jira Service Management API response for a rotation."""
