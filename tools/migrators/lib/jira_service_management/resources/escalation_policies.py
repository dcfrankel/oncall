...
def filter_escalation_policies(policies: list[dict]) -> list[dict]:
    ...
    if JIRA_SERVICE_MANAGEMENT_FILTER_TEAM:
        filtered_policies = []
        for p in policies:
            if p["ownerTeam"]["id"] == JIRA_SERVICE_MANAGEMENT_FILTER_TEAM:
                filtered_policies.append(p)
        policies = filtered_policies

    if JIRA_SERVICE_MANAGEMENT_FILTER_ESCALATION_POLICY_REGEX:
        pattern = re.compile(JIRA_SERVICE_MANAGEMENT_FILTER_ESCALATION_POLICY_REGEX)
...
def match_escalation_policy(policy: dict, oncall_escalation_chains: List[dict]) -> None:
    """
    Match Jira Service Management escalation policy with Grafana OnCall escalation chain.
    """
    oncall_chain = None
